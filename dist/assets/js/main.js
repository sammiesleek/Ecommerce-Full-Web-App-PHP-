// loading subcategories in headers

if (!localStorage.getItem("productsid")) {
  localStorage.setItem("productsid", JSON.stringify(["remove"]));
}
const proid = JSON.parse(localStorage.getItem("productsid"));
...
(function ($) {
  ...
  $(".add-to-cart").click(function () {
    var proids = JSON.parse(localStorage.getItem("productsid"));
    var dataid = $(this).attr("data-productid");
    console.log(dataid);
    const idexist = proids.find((id) => id === dataid);

    if (!idexist) {
      var purged = proids.filter((item) => item !== "remove");
  ...
  setTimeout(() => {
    $(".removefmcart").click(function () {
      var inproid = JSON.parse(localStorage.getItem("productsid"));
      var target = $(this);
      var proidd = target.attr("data-product_id");
      ...
      if (
        proidd ===
        $("#cartpgbd").children().find(target).attr("data-product_id")
      ) {
  ...
  setTimeout(() => {
    ...
    $("body").on("click", ".ec_qtybtn", function () {
      ...
      var maxquantity = parseFloat(
        $qtybutton.parent().find("input").attr("data-productquantity")
      );
  ...
  setTimeout(() => {
    ...
    $(".cart-qty-plus-minus .ec_cart_qtybtn .ec_qtybtn").on(
      ...
      function () {
        ...
        var subtotal = $cartqtybutton
          .parent()
          .parent()
          .parent()
          .parent()
          .find(".ec-cart-pro-subtotal");

        var proprice = $cartqtybutton
          .parent()
          .parent()
          .parent()
          .parent()
        ...
        var newsubtotal =
          $cartqtybutton.parent().parent().find("input").val() * proprice;
        subtotal.text("$" + parseFloat(newsubtotal));
      }
    );
  }, 1000);
